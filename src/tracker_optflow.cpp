...
{
    tracker_optflow::tracker_optflow(int _gpu_id, int win_size, int max_level, int iterations, int _flow_error) :
    ...
    void tracker_optflow::update_cur_bbox_vec(std::vector<tc::bounding_box> _cur_bbox_vec)
    {
        ...
        for (auto &i : cur_bbox_vec) {
            float x_center = (i.x + i.width / 2.0F);
            float y_center = (i.y + i.height / 2.0F);
    ...
    void tracker_optflow::update_tracking_flow(cv::Mat src_mat, std::vector<tc::bounding_box> _cur_bbox_vec)
    {
        int const old_gpu_id = cv::cuda::getDevice();
        if (old_gpu_id != _gpu_id)
            cv::cuda::setDevice(_gpu_id);
    ...
    std::vector<tc::bounding_box> tracker_optflow::tracking_flow(cv::Mat dst_mat, bool check_error)
    {
        ...
        std::vector<tc::bounding_box> result_bbox_vec;
        ...
        if (err_cpu.cols == cur_bbox_vec.size() && status_cpu.cols == cur_bbox_vec.size())
        {
            for (size_t i = 0; i < cur_bbox_vec.size(); ++i)
            {
                ...
                if (abs(moved_x) < 100 && abs(moved_y) < 100 && good_bbox_vec_flags[i])
                    if (err_cpu.at<float>(0, i) < _flow_error && status_cpu.at<unsigned char>(0, i) != 0 &&
